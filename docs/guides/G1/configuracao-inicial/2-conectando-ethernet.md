...
5. E se o IP for diferente de 192.168.123.X:
   ...
   ip -4 addr
   ```
   E procure pelo nome de interface de rede cabeada (como `enp...` ou `eth...`.

   Agora, execute:
   ``` 
   sudo ip addr flush dev "NOME_INTERFACE_REDE" 
   sudo ip addr add 192.168.123.200/24 dev "NOME_INTERFACE_REDE" 
   sudo ip link set "NOME_INTERFACE_REDE" up
   ```

   É importante notar que para o IP `192.168.123.X`, X não pode ser igual à `164`, `161` e nem `0`, pois são sub-redes reservadas pela Unitree para os controles do G1.

   Agora verifique:
   ```
   ip addr show "NOME_INTERFACE_REDE"
   ```
